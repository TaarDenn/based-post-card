...
// File: contracts/BasedPostCard.sol
...
contract BasedPostCard is ERC721AQueryable, Ownable, ReentrancyGuard {
    error OutOfPostcards();
    error LowEth();
    error NoTokenFound();
    error WithdrawFailed();

    IBasePixelFont public font;
    string public constant TOKEN_NAME = "PostCard";
    string public constant TOKEN_SYMBOL = "POSTCARD";
    uint256 public constant TOTAL_SUPPLY = 2 ** 256 - 1;
    address public constant DEV = 0x72DA5272B044c72a258dA0089c0Af53Be0ae5915;

    mapping(uint256 => Card) card;

    event Posted(address sender, address reciever, uint256 tokenId);
    ...
    function drawCard(uint256 tokenId) internal view returns (bytes memory) {
        bytes
            memory cardImageURI = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 400 600"><style>';

        cardImageURI = abi.encodePacked(
            cardImageURI,
        ...
        cardImageURI = abi.encodePacked(
            ...
            string(toColor(abi.encodePacked(card[tokenId].bg))),
            '" />'
        );

        cardImageURI = abi.encodePacked(cardImageURI, drawTexts(tokenId));
        cardImageURI = abi.encodePacked(cardImageURI, drawPixelCanvas(tokenId));
        cardImageURI = abi.encodePacked(cardImageURI, "</svg>");

        return cardImageURI;
    }

    function drawTexts(uint256 tokenId) internal view returns (bytes memory) {
        Card memory newCard = card[tokenId];

        bytes memory textParts = "";

        unchecked {
            for (uint256 i = 0; i < 3; i++) {
                if (bytes(newCard.texts[i]).length != 0) {
                    textParts = abi.encodePacked(
                        textParts,
                        '<text x="',
                        _toString(newCard.textPositions[i].x),
                        '" y="',
                        _toString(newCard.textPositions[i].y),
                        '" class="txt" fill="',
                        string(
                            toColor(abi.encodePacked(newCard.textColors[i]))
                        ),
                        '">',
                        newCard.texts[i],
                        "</text>"
    ...
    function drawPixelCanvas(
        ...
        bytes memory pixelCanvas = abi.encodePacked(
            ...
            _toString(newCard.pixelCanvasPosition.x),
            ",",
            _toString(newCard.pixelCanvasPosition.y),
            ')">'
        );
            ...
            for (uint256 x = 0; x < 8; x++) {
                for (uint256 y = 0; y < 8; y++) {
                    pixelCanvas = abi.encodePacked(
                        ...
                        ' fill="',
                        string(
                            toColor(abi.encodePacked(newCard.pixelCanvas[x][y]))
                        ),
                        '" />'
                    );
                }
    ...
    function mintAndPost(
        ...
        if (TOTAL_SUPPLY == _totalMinted()) revert OutOfPostcards();
        if (msg.value < _donationAmount + _giftAmount) revert LowEth();

        Card storage newCard = card[_nextTokenId()];

        newCard.name = _name;
        newCard.bg = _bg;
        newCard.pixelCanvas = _pixelCanvas;
        newCard.texts = _texts;
        newCard.textColors = _textColors;
        newCard.pixelCanvasPosition = _pixelCanvasPosition;

        newCard.textPositions[0] = Position(
            _textPositions[0].x,
            _textPositions[0].y
        );

        newCard.textPositions[1] = Position(
            _textPositions[1].x,
            _textPositions[1].y
        );

        newCard.textPositions[2] = Position(
            _textPositions[2].x,
            _textPositions[2].y
        );

        newCard.originalSender = msg.sender;
        newCard.originalReciever = _to;

        _withdraw(payable(_to), _giftAmount);
        _withdraw(payable(DEV), _donationAmount);

        emit Posted(msg.sender, _to, _totalMinted());
        _safeMint(_to, 1);
    }
    ...
    function getRawCard(uint256 _tokenId) public view returns (Card memory) {
        return card[_tokenId];
    }
    ...
    function tokenURI(
        ...
        metadata = abi.encodePacked(
            metadata,
            '"description": "Craft and post your hand-made NFT as a Postcard to your beloved one! On Base!", '
        );

        metadata = abi.encodePacked(
            metadata,
            '"image": "data:image/svg+xml;base64,',
            Base64.encode(drawCard(tokenId)),
            '"}'
        );
    ...
    function getSentCardsOf(
        address _owner
    ) public view returns (Card[] memory) {
        uint256 sentCardsCount = 0;
        uint256 currentIndex = 0;

        uint256 totalMinted = _totalMinted();

        for (uint256 i = 0; i < totalMinted; ++i) {
            if (card[i].originalSender == _owner) ++sentCardsCount;
        }

        Card[] memory sentCards = new Card[](sentCardsCount);

        if (sentCardsCount > 0) {
            for (uint256 i = 0; i < totalMinted; ++i) {
                if (card[i].originalSender == _owner) {
                    Card storage sentCard = card[i];
                    sentCards[currentIndex] = sentCard;
                    currentIndex++;
                }
            }
        }
    ...
    function getRecievedCardsOf(
        address _owner
    ) public view returns (Card[] memory) {
        uint256 recievedCardsCount = 0;
        uint256 currentIndex = 0;

        uint256 totalMinted = _totalMinted();

        for (uint256 i = 0; i < totalMinted; ++i) {
            if (card[i].originalSender == _owner) ++recievedCardsCount;
        }

        Card[] memory recievedCards = new Card[](recievedCardsCount);

        if (recievedCardsCount > 0) {
            for (uint256 i = 0; i < totalMinted; ++i) {
                if (card[i].originalReciever == _owner) {
                    Card storage recievedCard = card[i];
                    recievedCards[currentIndex] = recievedCard;
                    currentIndex++;
                }
            }
        }
